...
class ActivationApiView(APIView):
    def get(self, request, token, *args, **kwargs):
        try:
            ...
            return Response({'details': 'your token has been expired.'}, status=status.HTTP_400_BAD_REQUEST)
        except InvalidSignatureError:
            return Response({'details': 'The token is not valid.'}, status=status.HTTP_400_BAD_REQUEST)
        except DecodeError:
            return Response({'details': 'The token is not valid.'}, status=status.HTTP_400_BAD_REQUEST)
